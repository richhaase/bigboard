...
- `src/main.rs`, `src/config.rs`: four CLI flags, strict JSON preferences, timezone, groups, paths and exclusions. No paths/group means GitHub with saved selection; explicit paths/group means local with no automatic GitHub fallback. No JSON export.
