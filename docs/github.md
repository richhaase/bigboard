...
Run `bigboard` to open your saved GitHub repositories automatically, or show the picker if you have not selected any. If a saved repository is unavailable, the picker opens for review instead of silently loading a smaller selection.
...
| `e` | Read the full error when discovery or saving fails |
...
In error details, `↑/↓` or `j/k` scroll by line, `PgUp/PgDn` by page, and `Home/End` jump to the beginning/end. `Esc` or `e` returns to the picker, `R` reloads discovery, and `q` or `Ctrl-C` quits. After correcting a selection-save error, return to the picker and press `Enter` to retry saving. While editing a search, character keys enter text; `Ctrl-C` still quits.
...
Recognized AI coauthors, including configured `ai_identities`, mark a commit as AI-assisted without receiving human coauthored participation credit. A recognized AI primary author retains authored credit. Built-in matching uses specific agent identities; a human's AI-company email domain alone does not mark them as AI-assisted.
...
After collection, “Updating contributors” appears while the loaded summaries are calculated in the background. The range controls remain available; on the board, you can also change bot inclusion or repository filters, refresh, or reopen the GitHub picker. A newer request supersedes the pending calculation, so old results cannot overwrite the new selection. `q` or `Ctrl-C` cancels and quits. While API requests are loading, `Esc` also quits; other board controls resume after collection.
...
Automated fixtures cover paginated discovery and summaries, exact date filtering, pinned heads, coverage reuse and extension, unavailable cache storage, account changes, failed requests, AI and human coauthor attribution, unknown merge counts, cancellation, background result supersession, and responsive terminal layouts. A separately invoked read-only smoke test checks the public Big Board repository using the current `gh` account:

```bash
cargo test --locked github::tests::live_public_summary_scan -- --ignored --nocapture
```
